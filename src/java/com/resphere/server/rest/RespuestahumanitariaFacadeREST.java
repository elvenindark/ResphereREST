...
public class RespuestahumanitariaFacadeREST extends AbstractFacade<Respuestahumanitaria> {
    ...
    @GET
    @Path("findAll/{idevento}")
    @Produces({"application/xml", "application/json"})
    public List<Respuestahumanitaria> findByIdevento(@PathParam("idevento") String idevento){
        List<Respuestahumanitaria> respuesta = getEntityManager().createNamedQuery("Respuestahumanitaria.findByIdevento").setParameter("idevento", idevento).getResultList();       
        if(respuesta!=null)
            return respuesta;
        return null;
    }
    ...
    public List<Respuestahumanitaria> findAll() {
        List<Respuestahumanitaria> respuesta = getEntityManager().createNamedQuery("Respuestahumanitaria.findAll").getResultList();       
        if(respuesta!=null){            
            for(Respuestahumanitaria response:respuesta)
                System.out.println("{"+response.getAplica()+";"+response.getIdevento()+";"+response.getObservacion()+"}");
            return respuesta;
        }
        return null;
